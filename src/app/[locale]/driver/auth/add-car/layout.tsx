...
export async function generateMetadata({ params }: TLayoutParamsPromise) {
  ...
  const t = await getTranslations({ locale, namespace: 'AddCarPageHeadSection' });
...
const AddCarPageLayout: TFCWithChildren<TLayoutParamsPromise> = async ({ children, params }) => {
  const { locale } = await params;
...
export default AddCarPageLayout;
